//
// FIR filter coefficients for 1.2MS/s to 16kS/s downsampling filters
//
// @author Johan Hedin
// @date   2021
//
...
#ifndef FS_01200_08BIT_DS_TO_00016_HPP
#define FS_01200_08BIT_DS_TO_00016_HPP

#include <vector>

// Summary
//
// This file contain filters that can be used for downsampling a 1.2MS/s 8-bit
// IQ stream from a RTL dongle to 16kS/s. The dynamic range of the 1.2MS/s
...
//
// Going from 1.2MS/s to 16kS/s requires a total reduction factor M of 75. This
// can be achieved with tree stages with M = 3, M = 5 and M = 5 in any order.
// The resulting dynamic range is ~68.75dB.
//
...
// from 1.2MS/s to 400kS/s (M = 3) and increases the dynamic range from 50dB
// to ~54.8dB.
//
// Care band is between 0 and 10kHz and we want the attenuation in the folding
// area (400 +/- 10 kHz in this case) to be more than 50dB.
//
...
//
//   > vectortocpp(c, "fs_01200_08bit_ds_lpf1_01200_to_00400")
//
static const std::vector<float> fs_01200_08bit_ds_lpf1_01200_to_00400 = {
     0.0000137262713506f,
     0.0014987912638232f,
     0.0128474010569802f,
     0.0503710213693594f,
...
// from 400kS/s to 80kS/s (M = 5) and increases the dynamic range from ~54.8dB
// to ~61.8dB.
//
// Care band is between 0 and 10kHz and we want the attenuation in the folding
...
//   > c = sincflt(19, fs, fcut+20, @chebwin, 50);
//
// Plot the filter response with the care band and alias zones marked with the
// fltbox function:
//
...
// Output the filter coefficients as a C++ float vector:
//
//   > vectortocpp(c, "fs_01200_08bit_ds_lpf2_00400_to_00080")
//
static const std::vector<float> fs_01200_08bit_ds_lpf2_00400_to_00080 = {
    -0.0015689596792550f,
    -0.0024762766140713f,
    -0.0014205974059769f,
     0.0053419181118151f,
...
// from 80kS/s to 16kS/s (M = 5) and increases the dynamic range from ~61.8dB
// to ~68.75dB.
//
// Care band is between 0 and 5kHz and we want the attenuation in the folding
// areas (16 +/- 5 kHz and 32 +/- 5 kHz in this case) to be more than 62dB.
//
// Calculate with octave. fs is the sampling frequency and fcut is the care
// band high cutoff frequency (both in kHz):
//
...
// Output the filter coefficients as a C++ float vector:
//
//   > vectortocpp(c, "fs_01200_08bit_ds_lpf3_00080_to_00016")
//
static const std::vector<float> fs_01200_08bit_ds_lpf3_00080_to_00016 = {
    -0.0001088165129735f,
    -0.0002922217387049f,
    -0.0005887023450137f,
    -0.0008045434010244f,
...
#endif // FS_01200_08BIT_DS_TO_00016_HPP
